...
function MenteeAssignmentDetail() {
  ...
  React.useEffect(() => {
    ...
    const todoId = Number(assignmentId)
    if (Number.isNaN(todoId)) return
    run(() => todosApi.getTodoDetail({ todoId }), {
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    try {
      if (hasExisting) {
        await todosApi.updateTodoVerification({ todoId }, formData)
      } else {
        await todosApi.uploadTodoVerification({ todoId }, formData)
      }
      fetchDetail()
    } catch {
  ...
  const handleDeleteVerification = async () => {
    ...
    setUploadError('')
    try {
      await todosApi.deleteTodoVerification({ todoId })
      fetchDetail()
    } catch {
        ...
        <FormSectionGroup className="flex flex-col gap-4">
          ...
          <FormSection title="학습지">
            {loading ? (
              ...
              <FileDownloadCard title="첨부된 학습지가 없습니다." />
            )}
          </FormSection>
          ...
          <FormSection title="과제 인증 업로드">
            ...
            {isUploading ? (
              <p className="text-[12px] font-semibold text-figma-typo-gray">업로드 중...</p>
            ) : uploadError ? (
              <p className="text-[12px] font-semibold text-figma-sub-color-2">{uploadError}</p>
            ) : null}
            {timerError ? (
              <p className="text-[12px] font-semibold text-figma-sub-color-2">{timerError}</p>
            ) : null}
          </FormSection>
        </FormSectionGroup>
        <ConfirmModal
